...
public class ConnectionHelper {
        ...
        try {
            ...
            if (state.connected && !roaming) {
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) {
                    @SuppressLint("MissingPermission") NetworkInfo ani = (cm == null ? null : cm.getActiveNetworkInfo());
                    if (ani != null)
                        state.roaming = ani.isRoaming();
                } else {
                    @SuppressLint("MissingPermission") Network active = (cm == null ? null : cm.getActiveNetwork());
                    if (active != null) {
                        @SuppressLint("MissingPermission") NetworkCapabilities caps = cm.getNetworkCapabilities(active);
                        if (caps != null)
                ...
                if (state.roaming != null && state.roaming && rlah)
                    try {
                        ...
                        if (tm != null) {
                            ...
                            Log.i("ConnectionHelper","Country SIM=" + sim + " network=" + network);
    ...
    private static Boolean isMetered(Context context) {
        ...
        ConnectivityManager cm = Helper.getSystemService(context, ConnectivityManager.class);
        if (cm == null) {
            Log.i("ConnectionHelper","isMetered: no connectivity manager");
            return null;
        }
        ...
        if (active == null) {
            Log.i("ConnectionHelper","isMetered: no active network");
            return null;
        }
        ...
        if (ani == null || ani.getState() != NetworkInfo.State.CONNECTED) {
            Log.i("ConnectionHelper","isMetered: no/connected active info ani=" + ani);
            if (ani == null ||
        ...
        if (caps == null) {
            Log.i("ConnectionHelper","isMetered: active no caps");
            return null; // network unknown
        }

        Log.i("ConnectionHelper","isMetered: active caps=" + caps);

        if (caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_VPN)) {
            if (!caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET)) {
                Log.i("ConnectionHelper","isMetered: no internet");
                return null;
            }
            if (require_validated &&
                    Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
                    !caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)) {
                Log.i("ConnectionHelper","isMetered: not validated");
                return null;
            }
        }

        if (!caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_RESTRICTED)) {
            Log.i("ConnectionHelper","isMetered: active restricted");
            return null;
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P &&
                !caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_FOREGROUND)) {
            Log.i("ConnectionHelper","isMetered: active background");
            return null;
        }

        if (vpn_only) {
            boolean vpn = vpnActive(context);
            Log.i("ConnectionHelper","VPN only vpn=" + vpn);
            if (!vpn)
                return null;
        }
        ...
        if (standalone_vpn ||
            ...
            Log.i("ConnectionHelper","isMetered: active not VPN metered=" + metered);
            return metered;
        }
        ...
        for (Network network : networks) {
            ...
            if (caps == null) {
                Log.i("ConnectionHelper","isMetered: no underlying caps");
                continue; // network unknown
            }

            Log.i("ConnectionHelper","isMetered: underlying caps=" + caps);

            if (!caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET)) {
                Log.i("ConnectionHelper","isMetered: underlying no internet");
                continue;
            }

            if (!caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_RESTRICTED)) {
                Log.i("ConnectionHelper","isMetered: underlying restricted");
                continue;
            }

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P &&
                    !caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_FOREGROUND)) {
                Log.i("ConnectionHelper","isMetered: underlying background");
                continue;
            }
            ...
            if (!caps.hasTransport(NetworkCapabilities.TRANSPORT_VPN) &&
                ...
                Log.i("ConnectionHelper","isMetered: underlying other transport");
                continue;
            }

            if (caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_VPN)) {
                underlying = true;
                Log.i("ConnectionHelper","isMetered: underlying is connected");

                if (caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_METERED)) {
                    Log.i("ConnectionHelper","isMetered: underlying is unmetered");
                    return false;
                }
            }
        }
        ...
        // Assume metered
        Log.i("ConnectionHelper","isMetered: underlying assume metered");
        return true;
    }
