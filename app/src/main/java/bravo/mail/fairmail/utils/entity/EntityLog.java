...
public class EntityLog {
    ...
    public static void log(final Context context, @NonNull Type type, EntityAccount account, String data) {
        if (account == null)
    ...
    private static void cleanup(final Context context, final long before) {
        Log.i("EntityLog","Log cleanup interval=" + LOG_CLEANUP_INTERVAL);
        ...
        while (true)
            try {
                ...
                db.setTransactionSuccessful();
                Log.i("EntityLog","Cleanup logs=" + logs + " before=" + new Date(before));
