...
public class EntityCertificate {
    ...
    public static String getKeyId(X509Certificate certificate) {
        try {
            ...
            Log.e("EntityCertificate",ex.toString());
            return null;
        }
    }
            ...
            if (altNames != null)
                for (List altName : altNames)
                    if (altName.get(0).equals(GeneralName.rfc822Name))
                        ...
                        Log.i("EntityCertificate","Alt type=" + altName.get(0) + " data=" + altName.get(1));
    ...
    public static List<String> getDnsNames(X509Certificate certificate) throws CertificateParsingException {
        List<String> result = new ArrayList<>();
            ...
            try {
                if (altName.get(0).equals(GeneralName.dNSName))
                    ...
                    if (altName.get(1) instanceof String)
                        ...
                        Log.e("EntityCertificate","GeneralName.iPAddress type=" + (val == null ? null : val.getClass()));
                    }
            } catch (Throwable ex) {
                Log.e("EntityCertificate",ex.toString());
            }
    ...
    public static boolean matches(String server, List<String> names) {
        for (String name : names)
            if (matches(server, name)) {
                Log.i("EntityCertificate","Trusted server=" + server + " name=" + name);
                return true;
            }
        return false;
    }
