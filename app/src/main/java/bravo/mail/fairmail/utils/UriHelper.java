...
public class UriHelper {
    ...
    public static String getRootDomain(Context context, String host) {
        if (host == null)
            return null;
        String tld = getTld(context, host);
        if (tld == null)
        ...
        if (len < 0) {
            Log.e("UriHelper","getRootDomain host=" + host + " tld=" + tld);
            return null;
        }
    ...
    public static boolean isTld(Context context, String host) {
        if (host == null)
            return false;
        String tld = getTld(context, host);
    ...
    public static String getEmailUser(String address) {
        if (address == null)
            return null;
    ...
    private static void ensureSuffixList(Context context) {
        synchronized (suffixList) {
            ...
            Log.i("UriHelper","Reading " + SUFFIX_LIST_NAME);
            try (InputStream is = context.getAssets().open(SUFFIX_LIST_NAME)) {
                ...
                Log.i("UriHelper",SUFFIX_LIST_NAME + "=" + suffixList.size());
            } catch (Throwable ex) {
                Log.e("UriHelper",ex.toString());
            }
        }
    }
        ...
        for (String key : url.getQueryParameterNames()) {
            ...
            if (PARANOID_QUERY.contains(lkey) ||
                ...
                for (String value : url.getQueryParameters(key)) {
                    Log.i("UriHelper","Query " + key + "=" + value);
