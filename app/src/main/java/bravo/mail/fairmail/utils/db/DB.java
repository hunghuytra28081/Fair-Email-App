package bravo.mail.fairmail.utils.db;

import static bravo.mail.fairmail.utils.ServiceAuthenticator.AUTH_TYPE_PASSWORD;
...
import bravo.mail.fairmail.utils.ImageHelper;
import bravo.mail.fairmail.utils.InternetAddressJson;
import bravo.mail.fairmail.utils.MessageHelper;
import bravo.mail.fairmail.utils.dao.DaoAccount;
import bravo.mail.fairmail.utils.dao.DaoAnswer;
import bravo.mail.fairmail.utils.dao.DaoAttachment;
import bravo.mail.fairmail.utils.dao.DaoCertificate;
import bravo.mail.fairmail.utils.dao.DaoContact;
import bravo.mail.fairmail.utils.dao.DaoFolder;
import bravo.mail.fairmail.utils.dao.DaoIdentity;
import bravo.mail.fairmail.utils.dao.DaoLog;
import bravo.mail.fairmail.utils.dao.DaoMessage;
import bravo.mail.fairmail.utils.dao.DaoOperation;
import bravo.mail.fairmail.utils.dao.DaoRule;
import bravo.mail.fairmail.utils.dao.DaoSearch;
import bravo.mail.fairmail.utils.entity.EntityAccount;
import bravo.mail.fairmail.utils.entity.EntityAnswer;
import bravo.mail.fairmail.utils.entity.EntityAttachment;
import bravo.mail.fairmail.utils.entity.EntityCertificate;
import bravo.mail.fairmail.utils.entity.EntityContact;
import bravo.mail.fairmail.utils.entity.EntityFolder;
import bravo.mail.fairmail.utils.entity.EntityIdentity;
import bravo.mail.fairmail.utils.entity.EntityLog;
import bravo.mail.fairmail.utils.entity.EntityMessage;
import bravo.mail.fairmail.utils.entity.EntityOperation;
import bravo.mail.fairmail.utils.entity.EntityRule;
import bravo.mail.fairmail.utils.entity.EntitySearch;
import bravo.mail.fairmail.utils.tuple.TupleAccountView;
import bravo.mail.fairmail.utils.tuple.TupleFolderView;
import bravo.mail.fairmail.utils.tuple.TupleIdentityView;
...
public abstract class DB extends RoomDatabase {
        ...
        if (BuildConfig.DEBUG) {
            ...
            if (dbfile.exists()) {
                try (SQLiteDatabase db = SQLiteDatabase.openDatabase(dbfile.getPath(), null, SQLiteDatabase.OPEN_READWRITE)) {
                    Log.i("DB123","Set PRAGMA wal_autocheckpoint=" + DB_CHECKPOINT);
    ...
    public static void setupViewInvalidation(Context context) {
        ...
        db.account().liveAccountView().observeForever(new Observer<List<TupleAccountView>>() {
            ...
            public void onChanged(List<TupleAccountView> accounts) {
                ...
                if (changed) {
                    Log.i("DB123","Invalidating account view");
                    last = accounts;
        ...
        db.identity().liveIdentityView().observeForever(new Observer<List<TupleIdentityView>>() {
            ...
            public void onChanged(List<TupleIdentityView> identities) {
                ...
                if (changed) {
                    Log.i("DB123","Invalidating identity view");
                    last = identities;
                    db.getInvalidationTracker().notifyObserversByTableNames("message");
                }
            }
        ...
        db.folder().liveFolderView().observeForever(new Observer<List<TupleFolderView>>() {
            ...
            public void onChanged(List<TupleFolderView> folders) {
                ...
                if (changed) {
                    Log.i("DB123","Invalidating folder view");
                    last = folders;
    ...
    static void createEmergencyBackup(Context context) {
        Log.i("DB123","Creating emergency backup");
        try {
            DB db = DB.getInstance(context);
    ...
    private static void checkEmergencyBackup(Context context) {
        try {
            ...
            if (dbfile.exists()) {
                Log.i("DB123","Emergency restore /dbfile");
                return;
            }

            File emergency = new File(context.getFilesDir(), "emergency.json");
            if (!emergency.exists()) {
                Log.i("DB123","Emergency restore /json");
                return;
            }

            DB db = DB.getInstance(context);
            if (db.account().getAccounts().size() > 0) {
                Log.e("DB123","Emergency restore /accounts");
                return;
            }

            Log.e("DB123","Emergency restore");
    ...
    public static synchronized DB getInstance(Context context) {
        ...
        if (sInstance != null &&
            ...
            try {
                Log.e("DB123","Orphan database instance pid=" + apid + "/" + sPid);
                sInstance = null;
            } catch (Throwable ex) {
                Log.e("DB123",ex.toString());
            }
        ...
        if (sInstance == null) {
            Log.i("DB123","Creating database instance pid=" + sPid);
            ...
            try {
                Log.i("DB123","Disabling view invalidation");
                ...
                Log.i("DB123","Disabled view invalidation");
            ...
            sInstance.getInvalidationTracker().addObserver(new InvalidationTracker.Observer(DB_TABLES) {
                ...
                public void onInvalidated(@NonNull Set<String> tables) {
                    Log.d("DB123","ROOM invalidated=" + TextUtils.join(",", tables));
                }
            });
        }
    ...
    private static Builder<DB> getBuilder(Context context) {
        try {
            ReLinker.log(new ReLinker.Logger() {
                @Override
                public void log(String message) {
                    Log.i("DB123","Relinker: " + message);
                }
        ...
        Log.i("DB123","DB query threads=" + threads + " wal=" + wal);
...
//                        Log.breadcrumb("Database", crumb);
                        ...
                        if (cache_size != null) {
                            ...
                            Log.i("DB123","Set PRAGMA cache_size=" + cache_size);
                        ...
                        Log.i("DB123","Set PRAGMA cache_spill=0");
                        ...
                        for (String pragma : DB_PRAGMAS)
                            try (Cursor cursor = db.query("PRAGMA " + pragma + ";")) {
                                Log.i("DB123","Get PRAGMA " + pragma + "=" + (cursor.moveToNext() ? cursor.getString(0) : "?"));
                            }
...
//        Log.breadcrumb("Migration", crumb);
    ...
    private static Builder<DB> migrate(final Context context, Builder<DB> builder) {
                                    ...
                                    if (cursor != null && cursor.moveToNext()) {
                                        ...
                                        if (!TextUtils.isEmpty(name)) {
                                            ...
                                            if (source.renameTo(target))
                                                Log.i("DB123","Renamed attachment=" + target.getName());
                                            else {
                                                Log.i("DB123","Unavailable attachment=" + source.getName());
                    ...
                    public void migrate(@NonNull SupportSQLiteDatabase db) {
                        ...
                        try {
                            ...
                            while (cursor != null && cursor.moveToNext()) {
                                ...
                                Log.i("DB123","MMM account=" + id + " uuid=" + uuid);
        ...
        db.getQueryExecutor().execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.i("DB123","PRAGMA wal_checkpoint=" + sb + " elapse=" + elapse);
                } catch (Throwable ex) {
                    Log.e("DB123",ex.toString());
                }
            }
