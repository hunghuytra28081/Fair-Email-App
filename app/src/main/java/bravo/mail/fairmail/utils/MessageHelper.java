...
import bravo.mail.fairmail.extension.MimeMessageEx;
...
import bravo.mail.fairmail.utils.tuple.TupleThreadInfo;
...
//        Log.breadcrumb("Build message", c);
        ...
        if (message.references != null) {
            ...
            while (references.length() > maxlen && sp > 0) {
                Log.i("MessageHelper","Dropping reference=" + references.substring(0, sp));
        ...
        if (message.headers == null || !Boolean.TRUE.equals(message.resend)) {
            ...
            for (Header header : headers)
                try {
                    ...
                    Log.e("MessageHelper",ex.toString());
                }
        ...
        if (message.dsn == null || EntityMessage.DSN_NONE.equals(message.dsn)) {
            ...
            for (final EntityAttachment attachment : attachments)
                if (EntityAttachment.PGP_SIGNATURE.equals(attachment.encryption)) {
                    Log.i("MessageHelper","Sending PGP signed message");
                    ...
                    for (final EntityAttachment content : attachments)
                        if (EntityAttachment.PGP_CONTENT.equals(content.encryption)) {
                            ...
                            if (TextUtils.isEmpty(micalg)) {
                                ...
                                Log.w("MessageHelper","PGP micalg missing type=" + attachment.type);
                            }
                    ...
                    Log.i("MessageHelper","Sending PGP encrypted message");
                    ...
                    Log.i("MessageHelper","Sending S/MIME signed message");
                    ...
                    for (final EntityAttachment content : attachments)
                        if (EntityAttachment.SMIME_CONTENT.equals(content.encryption)) {
                            ...
                            if (TextUtils.isEmpty(micalg)) {
                                // Some providers strip parameters
                                Log.w("MessageHelper","S/MIME micalg missing type=" + attachment.type);
                                micalg = "sha-256";
                            }
                            ParameterList params = cts.getParameterList();
                            if (params != null)
                    ...
                    Log.i("MessageHelper","Sending S/MIME encrypted message");
    ...
    static Address getFrom(EntityMessage message, EntityIdentity identity) throws UnsupportedEncodingException {
        ...
        if (identity != null && identity.sender_extra &&
            ...
            if (!message.extra.equals(username)) {
                ...
                Log.i("MessageHelper","extra=\"" + name + "\" <" + email + ">");
            }
        }
    ...
    static AddressFormat getAddressFormat(Context context) {
        ...
        int email_format = prefs.getInt("email_format", name_email
                ? AddressFormat.NAME_EMAIL.ordinal()
                : AddressFormat.NAME_ONLY.ordinal());
        if (email_format < AddressFormat.values().length)
            return AddressFormat.values()[email_format];
        else
            return AddressFormat.NAME_ONLY;
    }
    ...
    private static class MimeTextPart {
        ...
        public String toString() {
            ...
            try {
                ...
                Log.e("MessageHelper", String.valueOf(new IllegalArgumentException(word, ex)));
                return word;
            }
        }
    }

    public static String getSortKey(Address[] addresses) {
        if (addresses == null || addresses.length == 0)
            return null;
